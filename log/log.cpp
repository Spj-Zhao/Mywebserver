...
bool Log::init(const char *file_name, int close_log, int log_buf_size, int split_lines, int max_queue_size) {
    if(max_queue_size > 1) {
        ...
        // 这个函数通常为新申请的内存做初始化工作， 其返回值为指向s的指针，它是对较大的结构体或数组进行清零操作的一种最快方法
...
void Log::write_log(int level, const char *format, ...) {
    ...
    // 它与 va_arg 和 va_end 宏是一起使用的。last_arg
    ...
    //nprintf和vsnprintf功能完全一样，只是vsnprintf将snprintf中的可变参数换成了av_list类型
    int m = vsnprintf(m_buff + n, m_log_buf_size - 1, format, valst);
    m_buff[n + m] = '\n';
    m_buff[n + m + 1] = '\0';
    log_str = m_buff;
    m_mutex.unlock();
    ...
    if(async_write_log() && !m_log_queue->full()){
